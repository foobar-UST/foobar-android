...
import com.foobarust.testshared.serialize.OrderSerialized
import com.foobarust.testshared.serialize.toOrderBasic
import com.foobarust.testshared.serialize.toOrderDetail
...
class FakeOrderRepositoryImpl : OrderRepository {
    ...
    val allOrderList: List<OrderSerialized> by lazy {
        deserializeJsonList("orders_fake_data.json")
    }

    val activeOrderList: List<OrderSerialized> by lazy {
        allOrderList.filter { it.state !in ORDER_HISTORY_STATES }
    }

    val historyOrderList: List<OrderSerialized> by lazy {
        allOrderList.filter { it.state in ORDER_HISTORY_STATES }
    }
